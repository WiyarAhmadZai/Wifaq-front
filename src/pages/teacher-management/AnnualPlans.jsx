...
const dummyData = [
  { id: 1, title: 'Annual Plan 2025', year: '2025', class: 'Class 1-A', subject: 'Mathematics', status: 'active' },
  { id: 2, title: 'Annual Plan 2025', year: '2025', class: 'Class 1-B', subject: 'English', status: 'active' },
];
  ...
  return (
    <div className="px-4 py-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div><h2 className="text-base font-bold text-gray-800">Annual Plans</h2><p className="text-xs text-gray-500 mt-0.5">Manage annual teaching plans</p></div>
        <div className="flex gap-2">
          <button onClick={() => navigate('/class-management/annual-plans/create')} className="px-3 py-1.5 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors flex items-center gap-1.5 font-medium text-xs"><Icons.Plus />Add New</button>
        </div>
      </div>
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div className="overflow-x-auto"><table className="w-full min-w-[800px]">
          <thead className="bg-teal-50"><tr>
            ...
            <th className="px-3 py-2 text-left text-[10px] font-semibold text-teal-800 uppercase tracking-wider">Year</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {items.map((item) => (<tr key={item.id} className="hover:bg-gray-50">
              ...
              <td className="px-3 py-2 text-xs text-gray-800">{item.year}</td>
              <td className="px-3 py-2 text-xs text-gray-800">{item.class}</td>
              <td className="px-3 py-2 text-xs text-gray-800">{item.subject}</td>
              <td className="px-3 py-2 text-center"><span className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-emerald-100 text-emerald-700">{item.status}</span></td>
              <td className="px-3 py-2 text-right"><div className="flex items-center justify-end gap-1">
                <button onClick={() => navigate(`/class-management/annual-plans/show/${item.id}`)} className="p-1 text-teal-600 hover:bg-teal-50 rounded"><Icons.Eye /></button>
                <button onClick={() => navigate(`/class-management/annual-plans/edit/${item.id}`)} className="p-1 text-amber-600 hover:bg-amber-50 rounded"><Icons.Edit /></button>
                <button onClick={() => handleDelete(item.id)} className="p-1 text-red-600 hover:bg-red-50 rounded"><Icons.Trash /></button>
              </div></td>
            </tr>))}
          </tbody>
        </table></div>
