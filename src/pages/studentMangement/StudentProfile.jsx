      ...
      <Card title={title} subtitle={subtitle} sensitive={sensitive}
        ...
        {list.length === 0 ? <Empty>No records yet.</Empty> : (
          <div className="space-y-3">
            {list.map((row, i) => (
              <div key={row.id || i} className="rounded-xl border border-gray-100 p-4">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  {row.context && <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-indigo-50 text-indigo-600">{row.context}</span>}
                  <span className="text-[11px] text-gray-400 ml-auto">{fmtDateTime(row[dateKey] || row.created_at)}</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-2">
                  {fields.filter((f) => f.bool ? row[f.name] : (row[f.name] !== null && row[f.name] !== "" && row[f.name] !== undefined)).map((f) => (
                    <div key={f.name}>
                      <p className="text-[10px] font-bold uppercase tracking-wide text-gray-400">{f.label}</p>
                      <p className="text-[13px] text-gray-800 mt-0.5 whitespace-pre-wrap" dir="auto">{f.bool ? "Yes" : labelFor(f, row[f.name])}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {ratings.map((r) => (
            <div key={r.id} className="rounded-xl border border-gray-100 p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-indigo-50 text-indigo-600">{r.context}</span>
                <span className="text-[11px] text-gray-400 ml-auto">{fmtDateTime(r.rated_at || r.created_at)}</span>
              </div>
              ...
              {r.evidence && <p className="text-[11px] text-gray-500 mt-2" dir="auto">Evidence: {r.evidence}</p>}
            </div>
          ))}
        </div>
      )}
