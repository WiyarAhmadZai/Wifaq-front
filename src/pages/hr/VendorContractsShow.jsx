...
export default function VendorContractsShow() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { canUpdate, canDelete } = useResourcePermissions("vendor-contracts");
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await del(`/hr/vendor-contracts/delete/${id}`);
        Swal.fire("Deleted", "Vendor contract deleted", "success");
  ...
  const submitRenew = async () => {
    ...
    setSavingRenew(true);
    try {
      await put(`/hr/vendor-contracts/renew/${id}`, { end_date: renewModal.end_date });
  ...
  if (!data) return (
    <div className="text-center py-12">
      <p className="text-gray-500">Vendor contract not found</p>
    ...
    <div className="w-full px-4 sm:px-6 py-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-800">{data.contract_number}</h2>
            <p className="text-xs text-gray-500">Vendor Contract Details</p>
          </div>
        </div>
        <div className="flex gap-2">
          ...
          {canUpdate && (
            <button onClick={() => navigate(`/hr/vendor-contracts/edit/${id}`)}
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-amber-700 mt-0.5">Notification was sent on {formatDate(data.renewal_alert_date)}. Renew or extend the contract to dismiss.</p>
                </div>
              </div>
            </div>
          )}
