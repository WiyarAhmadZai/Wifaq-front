...
const L = {
  en: {
    ...
    step2: "About You", step2d: "Personal information & introduction",
    step3: "Social", step3d: "Social media profiles (optional)",
    step4: "Motivation", step4d: "Why this role and what you bring",
    step5: "Education", step5d: "Your educational background",
    step6: "Experience", step6d: "Work history & responsibilities",
    step7: "Documents", step7d: "Upload your CV and credentials",
  ...
  ps: {
    ...
    step2: "ستاسو په اړه", step2d: "شخصي معلومات او پیژندنه",
    step3: "ټولنیزې رسنۍ", step3d: "د ټولنیزو رسنیو پروفایلونه (اختیاري)",
    step4: "انګیزه", step4d: "ولې دا بست او څه وړاندې کوئ",
    step5: "زده‌کړې", step5d: "ستاسو تحصیلي شالید",
    step6: "تجربه", step6d: "د کار سابقه او مسؤلیتونه",
    step7: "اسناد", step7d: "خپل سي‌وي او اسناد پورته کړئ",
  ...
  fa: {
    ...
    step2: "درباره شما", step2d: "معلومات شخصی و معرفی",
    step3: "شبکه‌های اجتماعی", step3d: "پروفایل‌های شبکه‌های اجتماعی (اختیاری)",
    step4: "انگیزه", step4d: "چرا این بست و چه چیزی ارائه می‌دهید",
    step5: "تحصیلات", step5d: "پیشینه تحصیلی شما",
    step6: "تجربه", step6d: "سابقه کاری و مسئولیت‌ها",
    step7: "اسناد", step7d: "بارگذاری رزومه و مدارک",
...
export default function PublicApplicationForm() {
  ...
  useEffect(() => {
    ...
    if (preselected) setFormData((prev) => ({ ...prev, job_posting_id: preselected }));
  }, []);
  ...
  const validateStep = () => {
    const e = {};
    if (step === 1 && !formData.job_posting_id) e.job_posting_id = t("e_job");
    if (step === 2) {
    ...
    if (step === 4 && !formData.motivation) e.motivation = t("e_motivation");
    if (step === 5) {
    ...
    if (step === 7) DOC_TYPES.forEach((d) => { if (d.required && !documents[d.key]) e[d.key] = t("e_required", { x: t(d.labelKey) }); });
    setErrors(e);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (status === 422 && error.response?.data?.errors) {
        ...
        const stepMap = { 1: ["job_posting_id"], 2: ["full_name", "contact_number", "email", "date_of_birth", "current_address", "place_of_origin", "introduction"], 3: ["facebook", "instagram", "twitter_x", "youtube"], 4: ["motivation"], 5: ["education_level", "field_of_study", "institution_name"], 6: ["total_experience_years"] };
          ...
          {step === 1 && (
            <StepCard icon={cur.icon} label={cur.label} desc={cur.desc}>
              <JobPicker t={t} jobPostings={jobPostings} selectedId={formData.job_posting_id}
                onSelect={(id) => { setFormData((p) => ({ ...p, job_posting_id: id })); if (errors.job_posting_id) setErrors((p) => ({ ...p, job_posting_id: null })); }}
          ...
          {step === 7 && (
            <StepCard icon={cur.icon} label={cur.label} desc={cur.desc}>
              ...
              <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-2.5 mt-4">
                ...
                {[
                  { label: t("s_job"), value: jobPostings.find((jp) => jp.id === parseInt(formData.job_posting_id))?.title || "—" },
...
function JobCard({ t, job, selected, onClick }) {
  ...
  let deadlineLabel = null, deadlineUrgent = false;
  if (job.deadline_date) {
    const d = new Date(job.deadline_date);
    if (!isNaN(d.getTime())) {
      const days = Math.ceil((d - new Date()) / (1000 * 60 * 60 * 24));
      if (days < 0) deadlineLabel = t("closed");
      else if (days === 0) { deadlineLabel = t("closes_today"); deadlineUrgent = true; }
      else if (days <= 7) { deadlineLabel = days === 1 ? t("day_left") : t("days_left", { n: days }); deadlineUrgent = true; }
      else deadlineLabel = t("closes_on", { d: d.toLocaleDateString("en-GB", { day: "2-digit", month: "short" }) });
    }
  }
  return (
