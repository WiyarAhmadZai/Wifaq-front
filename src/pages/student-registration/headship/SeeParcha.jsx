...
export default function SeeParcha() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const apiUrl = queryString ? `/api/see-parcha/list?${queryString}` : '/api/see-parcha/list';
  ...
  const handleCreate = () => {
    navigate('/student-registration/headship/see-parcha/create');
  };

  const handleEdit = (item) => {
    navigate(`/student-registration/headship/see-parcha/edit/${item.id}`);
  };

  const handleView = (item) => {
    navigate(`/student-registration/headship/see-parcha/show/${item.id}`);
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await del(`/api/see-parcha/delete/${id}`);
  ...
  const handleStatusUpdate = async () => {
    ...
    setSaving(true);
    try {
      await put(`/api/see-parcha/update-status/${selectedItem.id}`, {
  ...
  return (
    <div className="px-4 py-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div>
          <h2 className="text-base font-bold text-gray-800">See Parcha</h2>
          <p className="text-xs text-gray-500 mt-0.5">Manage student see parcha documents</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="md:col-span-2">
          ...
          <input
            ...
            placeholder="Search by student name or parcha number..."
            className="w-full px-2.5 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 text-xs"
          />
        </div>
      </div>
            ...
            <table className="w-full min-w-[800px]">
              <thead className="bg-teal-50">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-teal-800 uppercase tracking-wider">Parcha Number</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-teal-800 uppercase tracking-wider">Issue Date</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-xs text-gray-800">{item.parcha_number}</td>
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <div className="px-3 py-2 bg-gray-50 rounded-lg text-sm">
                  {selectedItem?.student_name} ({selectedItem?.parcha_number})
                </div>
              </div>
            </div>
