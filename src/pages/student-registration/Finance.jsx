...
const Icons = {
  Plus: () => (
    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.657 0-3-.895-3-2s1.343-2 3-2m0 8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
    </svg>
  ),
  Eye: () => (
    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function Finance() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const apiUrl = queryString ? `/api/finance/list?${queryString}` : '/api/finance/list';
  ...
  const handleCreate = () => {
    navigate('/student-registration/finance/create');
  };

  const handleEdit = (item) => {
    navigate(`/student-registration/finance/edit/${item.id}`);
  };

  const handleView = (item) => {
    navigate(`/student-registration/finance/show/${item.id}`);
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await del(`/api/finance/delete/${id}`);
  ...
  const handleStatusUpdate = async () => {
    ...
    setSaving(true);
    try {
      await put(`/api/finance/update-status/${selectedItem.id}`, {
    ...
    <div className="px-4 py-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Manage student fee payments and financial records</p>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[800px]">
              <thead className="bg-teal-50">
                <tr>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-teal-800 uppercase tracking-wider">Student Name</th>
                  ...
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-teal-800 uppercase tracking-wider">Payment Date</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-teal-800 uppercase tracking-wider">Due Date</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    <td className="px-3 py-2 text-xs font-medium text-teal-600">{item.student_name}</td>
                    <td className="px-3 py-2 text-xs text-gray-800">${item.amount.toLocaleString()}</td>
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <div className="px-3 py-2 bg-gray-50 rounded-lg text-sm">
                  {selectedItem?.student_name} (${selectedItem.amount ? '$' + selectedItem.amount.toLocaleString() : 'N/A'})
                </div>
              </div>
            </div>
