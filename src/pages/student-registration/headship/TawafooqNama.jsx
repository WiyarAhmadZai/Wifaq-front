...
export default function TawafooqNama() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const apiUrl = queryString ? `/api/tawafooq-nama/list?${queryString}` : '/api/tawafooq-nama/list';
  ...
  const handleCreate = () => {
    navigate('/student-registration/headship/tawafooq-nama/create');
  };

  const handleEdit = (item) => {
    navigate(`/student-registration/headship/tawafooq-nama/edit/${item.id}`);
  };

  const handleView = (item) => {
    navigate(`/student-registration/headship/tawafooq-nama/show/${item.id}`);
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await del(`/api/tawafooq-nama/delete/${id}`);
  ...
  const handleStatusUpdate = async () => {
    ...
    setSaving(true);
    try {
      await put(`/api/tawafooq-nama/update-status/${selectedItem.id}`, {
  ...
  return (
    <div className="px-4 py-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div>
          <h2 className="text-base font-bold text-gray-800">Tawafooq Nama</h2>
          <p className="text-xs text-gray-500 mt-0.5">Manage student headship agreements</p>
        </div>
            ...
            <table className="w-full min-w-[800px]">
              <thead className="bg-teal-50">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-teal-800 uppercase tracking-wider">Father Name</th>
                  <th className="px-3 py-2 text-left text-[10px] font-semibold text-teal-800 uppercase tracking-wider">Agreement Date</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 text-xs text-gray-800">{item.father_name}</td>
            ...
            <div className="p-4 space-y-4">
              ...
              <div>
                ...
                <div className="px-3 py-2 bg-gray-50 rounded-lg text-sm">
                  {selectedItem?.student_name} ({selectedItem?.father_name})
                </div>
              </div>
            </div>
