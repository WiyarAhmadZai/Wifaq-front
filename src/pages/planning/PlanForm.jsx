...
const STEPS = ["Frame", "Goals & items", "Review & submit"];
...
export default function PlanForm() {
  ...
  const save = async ({ submit }) => {
    ...
    try {
      ...
      let warning = null;
      if (submit && planId) {
        const r = await submitPlan(planId);
        warning = r.data?.warning;
      }
      await Swal.fire({
        icon: warning ? "warning" : "success",
        title: submit ? "Submitted for approval" : "Saved",
  ...
  return (
    <div className="px-4 py-5">
      <PageHeader
        ...
        subtitle="3-step wizard · Frame → Goals → Review"
        icon={ICON}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-4">
        <div>
          {/* STEP 1 — FRAME */}
          {step === 0 && (
            <Section title="Frame the plan" subtitle="Period, scope and intent" icon={ICON}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2">
                  <label className={label}>Plan title <span className="text-gray-300 normal-case">(optional — auto-generated)</span></label>
                  <input className={input} value={form.title} onChange={(e) => setField("title", e.target.value)} placeholder="Leave blank to auto-name from type + period" />
                </div>
                <div>
                ...
                <div>
                  <label className={label}>Period year (Shamsi)</label>
                ...
                {form.type === "weekly" && (
                  <div>
                    <label className={label}>Week (1–5)</label>
                    <input type="number" min="1" max="5" className={input} value={form.period_week} onChange={(e) => setField("period_week", e.target.value)} />
                  </div>
                )}
                {form.type !== "annual" && (
                  <div className="sm:col-span-2">
                    <label className={label}>Inherits from ({form.type === "monthly" ? "annual" : "monthly"} plan)</label>
                    <Select2 value={form.parent_id} onChange={(v) => setField("parent_id", v)}
                      ...
                      placeholder="Search parent plan…" />
                  </div>
                )}
                <div>
                ...
                <div className="sm:col-span-2">
                  <label className={label}>Narrative / intent</label>
                  <textarea className={input} rows={3} value={form.narrative} onChange={(e) => setField("narrative", e.target.value)} placeholder="What is this plan trying to achieve this period?" />
                </div>
              </div>
            </Section>
          )}
          ...
          {step === 1 && (
            <>
              <Section title="Objectives & key results" subtitle="2–5 objectives, each with measurable KRs (baseline · target · unit)" icon="M9 12l2 2 4-4"
                action={<button onClick={() => setObjectives((o) => [...o, blankObjective()])} className="text-xs font-semibold text-teal-600 hover:text-teal-800">+ Objective</button>}>
                <div className="space-y-4">
                  {objectives.map((o, oi) => (
                    <div key={oi} className="rounded-xl border border-gray-150 bg-gray-50/60 p-4">
                      <div className="flex gap-2 items-start mb-3">
                        <input className={input} value={o.statement} onChange={(e) => updObjective(oi, { statement: e.target.value })} placeholder={`Objective ${oi + 1} statement`} />
                        <select className="px-2 py-2 border border-gray-200 rounded-lg text-xs" value={o.primary_4d_dimension} onChange={(e) => updObjective(oi, { primary_4d_dimension: e.target.value })}>
                        ...
                        {o.key_results.map((k, ki) => (
                          <div key={ki} className="bg-white rounded-lg border border-gray-150 p-2.5">
                            <div className="flex gap-2 items-center">
                              <input className="flex-1 px-2.5 py-1.5 border border-gray-200 rounded-lg text-xs" value={k.statement} onChange={(e) => updKr(oi, ki, { statement: e.target.value })} placeholder={`Key result ${ki + 1}`} />
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-6 gap-1.5 mt-1.5">
                              <select className="px-2 py-1 border border-gray-200 rounded text-[11px]" value={k.kr_type} onChange={(e) => updKr(oi, ki, { kr_type: e.target.value })}>
                                {KR_TYPES.map((t) => <option key={t.value} value={t.value}>{t.label}</option>)}
                              </select>
                              <input className="px-2 py-1 border border-gray-200 rounded text-[11px]" value={k.baseline} onChange={(e) => updKr(oi, ki, { baseline: e.target.value })} placeholder="Baseline" />
                              <input className="px-2 py-1 border border-gray-200 rounded text-[11px]" value={k.target} onChange={(e) => updKr(oi, ki, { target: e.target.value })} placeholder="Target" />
                              <input className="px-2 py-1 border border-gray-200 rounded text-[11px]" value={k.unit} onChange={(e) => updKr(oi, ki, { unit: e.target.value })} placeholder="Unit" />
                              <input className="px-2 py-1 border border-gray-200 rounded text-[11px]" value={k.current_value} onChange={(e) => updKr(oi, ki, { current_value: e.target.value })} placeholder="Current" />
                              <input type="number" step="0.1" min="0" max="1" className="px-2 py-1 border border-gray-200 rounded text-[11px]" value={k.confidence_score} onChange={(e) => updKr(oi, ki, { confidence_score: e.target.value })} placeholder="Conf 0-1" />
                            </div>
                          </div>
                        ))}
                        <button onClick={() => updObjective(oi, { key_results: [...o.key_results, blankKr()] })} className="text-[11px] font-semibold text-teal-600 hover:text-teal-800">+ Key result</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <Section title="Plan items" subtitle="Each item becomes a Task on approval · +Event if dated · +Budget request if amount-tagged" icon="M4 6h16M4 12h16M4 18h7"
                action={<button onClick={() => setItems((it) => [...it, blankItem()])} className="text-xs font-semibold text-teal-600 hover:text-teal-800">+ Item</button>}>
                <div className="space-y-2">
                  {items.map((it, ii) => (
                    <div key={ii} className="rounded-xl border border-gray-150 bg-gray-50/60 p-3">
                      <div className="flex gap-2 items-center">
                        <input className={input} value={it.title} onChange={(e) => setItems((x) => x.map((y, i) => i === ii ? { ...y, title: e.target.value } : y))} placeholder={`Item ${ii + 1} title`} />
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 mt-1.5">
                        <div className="col-span-2 sm:col-span-1">
                          <Select2 size="sm" value={it.assigned_staff_id} onChange={(v) => setItems((x) => x.map((y, i) => i === ii ? { ...y, assigned_staff_id: v } : y))}
                            options={staff.map((s) => ({ value: s.id, label: s.name }))} placeholder="Assign to…" />
                        </div>
                        <DateField name={`due_${ii}`} value={it.due_date} onChange={(e) => setItems((x) => x.map((y, i) => i === ii ? { ...y, due_date: e.target.value } : y))} className="px-2 py-1.5 border border-gray-200 rounded text-[11px]" placeholder="Due (→ Event)" />
                        <input className="px-2 py-1.5 border border-gray-200 rounded text-[11px]" value={it.driver} onChange={(e) => setItems((x) => x.map((y, i) => i === ii ? { ...y, driver: e.target.value } : y))} placeholder="Driver" />
                        <input type="number" min="0" className="px-2 py-1.5 border border-gray-200 rounded text-[11px]" value={it.budget_amount} onChange={(e) => setItems((x) => x.map((y, i) => i === ii ? { ...y, budget_amount: e.target.value } : y))} placeholder="Budget AFN (→ request)" />
                      </div>
                      <input className="w-full px-2 py-1.5 border border-gray-200 rounded text-[11px] mt-1.5" value={it.description} onChange={(e) => setItems((x) => x.map((y, i) => i === ii ? { ...y, description: e.target.value } : y))} placeholder="Description / prep notes (optional)" />
                    </div>
                  ))}
                </div>
              </Section>
          ...
          {/* STEP 3 — REVIEW */}
          {step === 2 && (
            <Section title="Review & submit" subtitle="Confirm the structure before saving" icon="M5 13l4 4L19 7">
              <div className="space-y-3 text-sm">
                <Row k="Title">{form.title || "(auto-generated)"}</Row>
                <Row k="Type / period">{form.type} · {form.period_year || "—"}{form.period_month ? ` · ${SHAMSI_MONTHS[form.period_month]}` : ""}{form.period_week ? ` · wk ${form.period_week}` : ""}</Row>
                <Row k="Objectives">{objectives.filter((o) => o.statement.trim()).length}</Row>
                <Row k="Key results">{objectives.reduce((a, o) => a + o.key_results.filter((k) => k.statement.trim()).length, 0)}</Row>
                <Row k="Items (cascade on approval)">{items.filter((it) => it.title.trim()).length}</Row>
                <Row k="Budget-tagged items">{items.filter((it) => it.budget_amount && Number(it.budget_amount) > 0).length}</Row>
              </div>
              <div className="mt-5 flex flex-col sm:flex-row gap-2">
                <button onClick={() => save({ submit: false })} disabled={saving} className="px-4 py-2.5 bg-white border border-teal-200 text-teal-700 text-sm font-semibold rounded-xl hover:bg-teal-50 disabled:opacity-50">{saving ? "Saving…" : "Save draft"}</button>
                <button onClick={() => save({ submit: true })} disabled={saving} className="px-4 py-2.5 bg-teal-600 text-white text-sm font-bold rounded-xl hover:bg-teal-700 disabled:opacity-50">{saving ? "Saving…" : "Save & submit for approval"}</button>
              </div>
            </Section>
          )}

          <div className="flex justify-between items-center mt-3">
            <button onClick={() => setStep((s) => Math.max(0, s - 1))} disabled={step === 0} className="px-4 py-2 bg-white border border-gray-200 text-gray-600 text-sm font-semibold rounded-xl disabled:opacity-40">← Previous</button>
        ...
        <div className="space-y-4">
          <Section title="4D balance check" icon="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z">
