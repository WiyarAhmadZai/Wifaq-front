...
export default function StaffForm() {
  ...
  const ic = (f) => `w-full px-2.5 py-2 border rounded-lg focus:ring-2 focus:outline-none text-xs transition-all ${err(f) ? "border-red-300 focus:ring-red-300 bg-red-50" : "border-gray-200 focus:ring-teal-400 hover:border-gray-300 bg-white"}`;
    ...
    <div className="flex gap-1.5">
      {options.map((opt) => (
        <button
          ...
          className={`flex-1 px-2 py-2 rounded-lg text-[11px] font-medium border transition-all ${
            value === opt.value ? `${opt.color} ring-1.5 ring-offset-1 ring-teal-400` : "bg-white text-gray-400 border-gray-200 hover:border-gray-300"
          }`}
        >{opt.label}</button>
      ))}
    </div>
  ...
  return (
    <div className="px-4 py-3 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <div>
          <h2 className="text-sm font-bold text-gray-800">{isEdit ? "Edit Staff" : "Register New Staff"}</h2>
          <p className="text-[10px] text-gray-400">Step {currentStep} of {steps.length}</p>
        </div>
      </div>
        ...
        {steps.map((step, i) => (
          <div key={step.id} className="flex items-center flex-1 min-w-0">
            <button
              ...
              className={`flex items-center gap-1.5 px-2 py-1.5 rounded-lg transition-all whitespace-nowrap ${
                currentStep === step.id ? "bg-teal-600 text-white shadow-sm" : currentStep > step.id ? "bg-teal-50 text-teal-700" : "bg-gray-50 text-gray-400"
              }`}
            >
              ...
              <span className="text-[10px] font-semibold hidden md:block">{step.title}</span>
            </button>
            {i < steps.length - 1 && <div className={`flex-1 h-px mx-1 ${currentStep > step.id ? "bg-teal-300" : "bg-gray-200"}`} />}
          </div>
        ))}
      </div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {currentStep === 1 && (
          <StepCard step={steps[0]} gradient="from-teal-50 to-emerald-50" subtitle="Basic personal identity details">
            ...
            <div className="flex items-center gap-4 mb-4 pb-4 border-b border-gray-100">
              <div className="w-14 h-14 rounded-xl bg-gray-100 border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden flex-shrink-0">
                {photoPreview ? (
                  <img src={photoPreview} alt="Preview" className="w-full h-full object-cover" />
                ) : (
                  <svg className="w-6 h-6 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                )}
              </div>
              <div>
                <label className="inline-flex items-center gap-1 px-2.5 py-1.5 bg-teal-50 text-teal-700 rounded-md text-[10px] font-semibold cursor-pointer hover:bg-teal-100 transition-all">
                ...
                <p className="text-[9px] text-gray-400 mt-0.5">JPG, PNG - Max 2MB</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <InputField label="National ID / Tazkira" name="national_id" value={formData.national_id} onChange={handleChange} required placeholder="e.g. 1401-0123-45678" error={err("national_id")} ic={ic} />
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 mb-1">Blood Type</label>
        ...
        {currentStep === 2 && (
          <StepCard step={steps[1]} gradient="from-teal-50 to-indigo-50" subtitle="Phone, email, and emergency contacts">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="sm:col-span-2">
                <label className="block text-[10px] font-semibold text-gray-500 mb-1">Home Address *</label>
                ...
                {err("home_address") && <p className="text-red-500 text-[9px] mt-0.5">{err("home_address")}</p>}
              </div>
            </div>
            ...
            <div className="mt-3 pt-3 border-t border-gray-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <InputField label="Contact Name" name="emergency_contact_name" value={formData.emergency_contact_name} onChange={handleChange} required placeholder="Contact person" error={err("emergency_contact_name")} ic={ic} />
        ...
        {currentStep === 3 && (
          <StepCard step={steps[2]} gradient="from-teal-50 to-cyan-50" subtitle="Job details, contract, and organization">
            {/* Staff Code */}
            <div className="bg-teal-50 rounded-lg p-3 mb-3 flex items-center justify-between border border-teal-100">
              <div>
                <p className="text-[9px] font-semibold text-teal-600 uppercase tracking-wider">Staff Code (Auto)</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                {err("department") && <p className="text-red-500 text-[9px] mt-0.5">{err("department")}</p>}
              </div>
            </div>

            <div className="mt-3">
              <label className="block text-[10px] font-semibold text-gray-500 mb-1">Organization *</label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
              ...
              <InputField label="Job Title (Dari)" name="job_title_dari" value={formData.job_title_dari} onChange={handleChange} placeholder="عنوان وظیفه" dir="rtl" ic={ic} />
            </div>

            <div className="mt-3">
              <label className="block text-[10px] font-semibold text-gray-500 mb-1">Contract Type *</label>
            ...
            <div className="mt-3">
              <label className="block text-[10px] font-semibold text-gray-500 mb-1">Employment Status *</label>
            ...
            <div className="mt-3 pt-3 border-t border-gray-100 grid grid-cols-2 gap-3">
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 mb-1">Direct Supervisor</label>
        ...
        {currentStep === 4 && (
          <StepCard step={steps[3]} gradient="from-teal-50 to-pink-50" subtitle="Initial salary and allowance setup">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 mb-1">Rank Level *</label>
            ...
            <div className="mt-3 pt-3 border-t border-gray-100">
              <p className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Allowances</p>
              <div className="grid grid-cols-3 gap-3">
                <InputField label="Housing (AFN)" name="housing_allowance" type="number" value={formData.housing_allowance} onChange={handleChange} placeholder="0" ic={ic} />
                <InputField label="Transport (AFN)" name="transport_allowance" type="number" value={formData.transport_allowance} onChange={handleChange} placeholder="0" ic={ic} />
                <InputField label="Family (AFN)" name="family_allowance" type="number" value={formData.family_allowance} onChange={handleChange} placeholder="0" ic={ic} />
              </div>
            </div>
            ...
            <div className="mt-3 bg-gray-50 rounded-lg p-3 border border-gray-100">
              <div className="grid grid-cols-5 gap-2">
                <div className="bg-white rounded-md p-2 text-center border border-gray-100">
                  <p className="text-[8px] font-bold text-gray-400 uppercase">Base</p>
                  <p className="text-xs font-bold text-gray-800">{(parseFloat(formData.base_salary) || 0).toLocaleString()}</p>
                </div>
                <div className="bg-white rounded-md p-2 text-center border border-blue-100">
                  <p className="text-[8px] font-bold text-blue-400 uppercase">Housing</p>
                  <p className="text-xs font-bold text-blue-600">+{(parseFloat(formData.housing_allowance) || 0).toLocaleString()}</p>
                </div>
                <div className="bg-white rounded-md p-2 text-center border border-purple-100">
                  <p className="text-[8px] font-bold text-purple-400 uppercase">Transport</p>
                  <p className="text-xs font-bold text-purple-600">+{(parseFloat(formData.transport_allowance) || 0).toLocaleString()}</p>
                </div>
                <div className="bg-white rounded-md p-2 text-center border border-amber-100">
                  <p className="text-[8px] font-bold text-amber-400 uppercase">Family</p>
                  <p className="text-xs font-bold text-amber-600">+{(parseFloat(formData.family_allowance) || 0).toLocaleString()}</p>
                </div>
        ...
        {/* Step 5: Documents */}
        {currentStep === 5 && (
          <StepCard step={steps[4]} gradient="from-teal-50 to-violet-50" subtitle="Upload required documents">
            <div className="space-y-2.5">
              {[
                { name: "cv_upload", label: "CV Upload", required: true, accept: ".pdf,.doc,.docx", desc: "PDF, DOC" },
                { name: "tazkira_scan", label: "Tazkira / National ID Scan", required: true, accept: ".pdf,.jpg,.jpeg,.png", desc: "PDF, JPG, PNG" },
                ...
                <div key={doc.name} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border transition-all ${formData[doc.name] ? 'bg-teal-50/50 border-teal-200' : 'border-gray-200 hover:border-teal-200'}`}>
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${formData[doc.name] ? "bg-teal-100 text-teal-600" : "bg-gray-100 text-gray-400"}`}>
                    {formData[doc.name] ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-gray-700">{doc.label} {doc.required && <span className="text-red-400">*</span>}</p>
                    <p className="text-[9px] text-gray-400 truncate">{formData[doc.name] ? formData[doc.name].name : doc.desc}</p>
                  </div>
                  <label className="px-2.5 py-1 bg-teal-50 text-teal-700 rounded-md text-[10px] font-semibold cursor-pointer hover:bg-teal-100 transition-all flex-shrink-0">
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-lg p-2.5 mt-3 flex items-start gap-2">
              <svg className="w-3.5 h-3.5 text-amber-500 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              <p className="text-[10px] text-amber-700">Signed contract can be uploaded later. All other documents are required.</p>
            </div>
          </StepCard>
        )}
        ...
        <div className="flex items-center justify-between mt-4">
          <button type="button" onClick={() => currentStep === 1 ? navigate("/hr/staff") : setCurrentStep(currentStep - 1)}
            className="px-3 py-1.5 bg-gray-100 text-gray-600 rounded-lg text-xs font-medium hover:bg-gray-200 transition-all flex items-center gap-1">
          ...
          {currentStep < 5 ? (
            <button type="button" onClick={() => setCurrentStep(currentStep + 1)}
              className="px-4 py-1.5 bg-teal-600 text-white rounded-lg text-xs font-medium hover:bg-teal-700 transition-all shadow-sm flex items-center gap-1">
              Next
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
            </button>
          ) : (
            <button type="submit" disabled={saving}
              className="px-4 py-1.5 bg-teal-600 text-white rounded-lg text-xs font-medium hover:bg-teal-700 transition-all shadow-sm disabled:opacity-50 flex items-center gap-1.5">
              {saving ? (
                <><div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>Saving...</>
              ) : (
                <><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>{isEdit ? "Update" : "Register"}</>
              )}
            </button>
          )}
        </div>
...
function StepCard({ step, gradient, subtitle, children }) {
  return (
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
      <div className={`px-4 py-2.5 bg-gradient-to-r ${gradient} border-b border-gray-100`}>
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 bg-teal-100 rounded-md flex items-center justify-center">
            <svg className="w-3 h-3 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={step.icon} /></svg>
          </div>
          <div>
            <h3 className="text-xs font-bold text-gray-800">{step.title}</h3>
...
function InputField({ label, name, type = "text", value, onChange, required, placeholder, error, ic, dir }) {
  return (
    <div>
      <label className="block text-[10px] font-semibold text-gray-500 mb-1">{label} {required && '*'}</label>
