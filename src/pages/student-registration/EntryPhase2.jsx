...
export default function EntryPhase2() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const apiUrl = queryString ? `/api/entry-phase-2/list?${queryString}` : '/api/entry-phase-2/list';
  ...
  const handleCreate = () => {
    navigate('/student-registration/entry-phase-2/create');
  };

  const handleEdit = (item) => {
    navigate(`/student-registration/entry-phase-2/edit/${item.id}`);
  };

  const handleView = (item) => {
    navigate(`/student-registration/entry-phase-2/show/${item.id}`);
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await del(`/api/entry-phase-2/delete/${id}`);
  ...
  const handleStatusUpdate = async () => {
    ...
    setSaving(true);
    try {
      await put(`/api/entry-phase-2/update-status/${selectedItem.id}`, {
  ...
  return (
    <div className="px-4 py-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div>
          <h2 className="text-base font-bold text-gray-800">Entry Phase 2</h2>
          <p className="text-xs text-gray-500 mt-0.5">Manage student registration entry phase 2 applications</p>
        </div>
