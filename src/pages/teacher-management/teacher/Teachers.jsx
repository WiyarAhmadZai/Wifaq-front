...
const dummyData = [
  { id: 1, name: 'Mr. Ahmad Khan', email: 'ahmad@school.com', subject: 'Mathematics', phone: '0300-1234567', status: 'active' },
  { id: 2, name: 'Ms. Fatima Ali', email: 'fatima@school.com', subject: 'English', phone: '0300-2345678', status: 'active' },
  { id: 3, name: 'Mr. Hassan Raza', email: 'hassan@school.com', subject: 'Science', phone: '0300-3456789', status: 'active' },
  { id: 4, name: 'Mrs. Aisha Mahmood', email: 'aisha@school.com', subject: 'Urdu', phone: '0300-4567890', status: 'inactive' },
];
...
export default function Teachers() {
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) { Swal.fire('Deleted!', 'Record has been deleted.', 'success'); }
  };

  return (
    <div className="px-4 py-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div><h2 className="text-base font-bold text-gray-800">Teachers</h2><p className="text-xs text-gray-500 mt-0.5">Manage teaching staff</p></div>
        <div className="flex gap-2">
          ...
          <button onClick={() => navigate('/class-management/teachers/create')} className="px-3 py-1.5 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors flex items-center gap-1.5 font-medium text-xs"><Icons.Plus />Add New</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4 p-4 bg-gray-50 rounded-lg">
        ...
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Search</label><input type="text" value={filters.search} onChange={(e) => setFilters({...filters, search: e.target.value})} placeholder="Search by name or subject..." className="w-full px-2.5 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500 text-xs" /></div>
      </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {items.map((item) => (<tr key={item.id} className="hover:bg-gray-50">
              ...
              <td className="px-3 py-2 text-right"><div className="flex items-center justify-end gap-1">
                <button onClick={() => navigate(`/class-management/teachers/show/${item.id}`)} className="p-1 text-teal-600 hover:bg-teal-50 rounded transition-colors"><Icons.Eye /></button>
                <button onClick={() => navigate(`/class-management/teachers/edit/${item.id}`)} className="p-1 text-amber-600 hover:bg-amber-50 rounded transition-colors"><Icons.Edit /></button>
